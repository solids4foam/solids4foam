...
)
{
    ...
    // cout<< "max memory access size is "
    //     << std::numeric_limits<int>::max()*sizeof(scalar) << endl;
    // cout<< "max memory access size is "
    //     << std::numeric_limits<long int>::max()*sizeof(scalar) << nl << endl;
    ...
    if (scale_)
    {
        Info<< "Scaling the matrix to equilibriate rows and column norms"
            << endl;

        // Direct sparse LU solver where an iterative scaling algorithm is
        // applied to equilibrate rows and column norms in matrices

        // Create scaling
        scalPtr = new Eigen::IterScaling<Eigen::SparseMatrix<double> >();

        // Compute the left and right scaling vectors.
        // The matrix is equilibrated at output
        scalPtr->computeRef(A);

        // Scale the right hand side
        b = scalPtr->LeftScaling().cwiseProduct(b);
    }
    ...
    if (scale_)
    {
        ...
        x = scalPtr->RightScaling().cwiseProduct(x);
    }
