...
{
    // Assume a constant distribution of material properties for the
    // high-order quadrature calculation.
    ...
    forAll(sigmaQuad, faceI)
    {
        ...
        forAll(faceSigmaQuad, qpI)
        {
            ...
            // Isochoric left Cauchy-Green deformation tensor
            const symmTensor isoB(pow(J, -2.0/3.0)*symm(F & F.T()));
            const symmTensor sqrB(symm(isoB & isoB));

            // Invariants
            const scalar I1(tr(isoB));
            const scalar I2(0.5*(pow(I1, 2.0) - tr(sqrB)));
            ...
            // Hydrostatic stress
            const scalar sigmaHyd = 0.5*K*(pow(J, 2.0) - 1.0);

            // Cauchy stress
            faceSigmaQuad[qpI] = (1.0/J)*(dev(s) + sigmaHyd*I);
        }
    }
}
#endif
