...
bool nonLinGeomTotalLagVelocitySolid::evolveImplicitSegregated()
{
...
{
    notImplemented("Change to velocity approach!");
...
{
    notImplemented("Incorporate Adams–Bashforth extrapolation");

    // TODO - incorporate below approach into velocity-based solid model, e.g.
    // nonLinGeomVelocityTotalLag
    // In the solid equations, 0.5*div(sigma[n] + sigma[n-1]) is used, where
    // sigma[n+1] is calculated using D[n+1] (D at the future time step - Dnew)
    // Approximate D[n+1] using a second-order Adams–Bashforth extrapolation of
    // the solid velocity field USolid. This is the approach given in Jaiman and
    // Joshi in Eq. 6.16.
    // In this way, sigma[n+1] depends on USolid[n] via Dnew
    // const volVectorField Dnew
    // (
    //     "Dnew", D + 0.5*deltaT*(3*USolid - USolid.oldTime())
    // );
