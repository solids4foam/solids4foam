...
```warning
`unsLinearGeometry` is **withdrawn**: it is not compiled on any fork, so
selecting it fails to construct the solid model. No tutorial selected it, so
...
lists. Reintroducing it needs a tutorial; see issue #461.
