...
- computes the displacement gradient and the stress at the **dual mesh faces**
  (`dualGradDf`, `dualSigmaf`), through the `mechanicalConstitutiveLaw`
  framework;
...
The whole constitutive response comes from the `mechanicalConstitutiveLaw`
framework, on the PETSc SNES and explicit paths alike, read from
`constant/mechanicalProperties`.

More than one material is supported, each dual face taking the law of the
primary cell it lies in.
...
- constitutive evaluation goes through the `mechanicalConstitutiveLaw`
  framework (see below);
...
solution field and `solutionLocation::POINTS`, computes `blockSize_`,
allocates the
