...
#include "vertexCentredNonLinGeomUpdatedLagSolid.H"
...
defineTypeNameAndDebug(vertexCentredNonLinGeomUpdatedLagSolid, 0);
addToRunTimeSelectionTable
(
    solidModel, vertexCentredNonLinGeomUpdatedLagSolid, dictionary
);

const Enum<vertexCentredNonLinGeomUpdatedLagSolid::solutionAlgorithm>
vertexCentredNonLinGeomUpdatedLagSolid::solutionAlgorithmNames_
({
    {
        vertexCentredNonLinGeomUpdatedLagSolid::solutionAlgorithm::IMPLICIT_COUPLED,
        "implicitCoupled"
    },
    {
        vertexCentredNonLinGeomUpdatedLagSolid::solutionAlgorithm::IMPLICIT_SEGREGATED,
        "implicitSegregated"
    },
    {
        vertexCentredNonLinGeomUpdatedLagSolid::solutionAlgorithm::EXPLICIT,
        "explicit"
    },
});
...
void vertexCentredNonLinGeomUpdatedLagSolid::updateSource
(
    vectorField& source,
...
)
{
    ...
    // F = div(J F^-T * sigma) + rho*g - rho*d2dt2(D)
    // Update Lagrangian form
    // F = div(d f^-T * sigma) + rho*g - rho*d2dt2(D)
    ...
    (
        dualRelJf_*dualRelFinvf_.T() & dualMesh().Sf()
    );
    ...
    if (debug)
    {
        Info<< "void vertexCentredNonLinGeomUpdatedLagSolid::"
            << "updateSource(...): end"
            << endl;
    }
}
...
void vertexCentredNonLinGeomUpdatedLagSolid::updatePointDivSigma
(
    const pointVectorField& pointD,
    pointVectorField& pointDD,
    surfaceTensorField& dualGradDDf,
    surfaceTensorField& dualRelFf,
    surfaceTensorField& dualFf,
    surfaceTensorField& dualRelFinvf,
    surfaceScalarField& dualRelJf,
...
)
{
    ...
    // Calculate gradDD at dual faces
    dualGradDDf = vfvc::fGrad
    (
        pointDD,
        mesh(),
        dualMesh(),
    ...
    // Update relF
    dualRelFf = I + dualGradDDf.T();

    // Update F
    dualFf = dualRelFf & dualFf.oldTime();

    // Update relFinv
    dualRelFinvf = inv(dualRelFf);

    // Update Finv
    // dualFinvf = inv(dualFf);

    // Update relJ
    dualRelJf = det(dualRelFf);
    ...
    const surfaceVectorField deformedDualSf
    (
        dualRelJf*dualRelFinvf.T() & dualMesh().Sf()
    );
    ...
    if (debug)
    {
        Info<< "void updatePointDivSigma(...): end" << endl;
    }
}


void vertexCentredNonLinGeomUpdatedLagSolid::setFixedDofs
(
    const pointVectorField& pointD,
    boolList& fixedDofs,
...
void vertexCentredNonLinGeomUpdatedLagSolid::enforceTractionBoundaries
(
    const pointVectorField& pointD,
    ...
    forAll(pointD.boundaryField(), patchI)
    {
        ...
        )
        {
            ...
            if (gMin(nPointsPerDualFace) < 1)
            {
                FatalErrorIn
                (
                    "void vertexCentredNonLinGeomUpdatedLagSolid::"
                    "enforceTractionBoundaries(...)"
...
bool vertexCentredNonLinGeomUpdatedLagSolid::
vertexCentredNonLinGeomUpdatedLagSolid::converged
(
    const label iCorr,
    scalar& initResidual,
    const scalar res,
...
scalar vertexCentredNonLinGeomUpdatedLagSolid::calculateLineSearchSlope
(
    const scalar eta,
    const vectorField& pointDcorr,
...
)
{
    ...
    // Update F
    dualRelFf_ = I + dualGradDDf.T();

    // Update F
    dualFf_ = dualRelFf_ & dualFf_.oldTime();

    // Update relFinv
    dualRelFinvf_ = inv(dualRelFf_);

    // Update Finv
    // dualFinvf = inv(dualFf);

    // Update relJ
    dualRelJf_ = det(dualRelFf_);
...
scalar vertexCentredNonLinGeomUpdatedLagSolid::calculateLineSearchFactor
(
...
vertexCentredNonLinGeomUpdatedLagSolid::geometricStiffnessField
(
...
void vertexCentredNonLinGeomUpdatedLagSolid::makeDualImpKf() const
{
...
vertexCentredNonLinGeomUpdatedLagSolid::dualImpKf() const
{
...
surfaceScalarField&
vertexCentredNonLinGeomUpdatedLagSolid::dualImpKf()
{
    if (dualImpKfPtr_.empty())
    {
        makeDualImpKf();
...
void vertexCentredNonLinGeomUpdatedLagSolid::predict()
{
...
bool vertexCentredNonLinGeomUpdatedLagSolid::evolveImplicitCoupled()
{
    ...
    // Store the dual area vectors (updated configuration)
    ...
    (
        geometricStiffnessField(dualSf, dualGradDDf_)
    );
    ...
    do
    {
        ...
        if (fullNewton_)
        {
            ...
            geometricStiffness = geometricStiffnessField(dualSf, dualGradDDf_);
        ...
        if (Switch(solidModelDict().lookup("usePETSc")))
        {
            ...
            FatalErrorIn
            (
                "vertexCentredNonLinGeomUpdatedLagSolid::evolve()"
        ...
        if (debug)
        {
            Info<< "bool vertexCentredNonLinGeomUpdatedLagSolid::evolve(): "
...
bool vertexCentredNonLinGeomUpdatedLagSolid::evolveImplicitSegregated()
{
    Info<< "Evolving solid solver" << endl;
...
bool vertexCentredNonLinGeomUpdatedLagSolid::evolveExplicit()
{
    if (time().timeIndex() == 1)
    {
...
vertexCentredNonLinGeomUpdatedLagSolid::
vertexCentredNonLinGeomUpdatedLagSolid
(
    Time& runTime,
    const word& region
...
vertexCentredNonLinGeomUpdatedLagSolid::~vertexCentredNonLinGeomUpdatedLagSolid()
{
#ifdef USE_PETSC
    if (Switch(solidModelDict().lookup("usePETSc")))
    {
...
void vertexCentredNonLinGeomUpdatedLagSolid::setDeltaT(Time& runTime)
{
...
bool vertexCentredNonLinGeomUpdatedLagSolid::evolve()
{
...
void vertexCentredNonLinGeomUpdatedLagSolid::setTraction
(
...
)
{
    ...
    else
    {
        FatalErrorIn
        (
            "void Foam::vertexCentredNonLinGeomUpdatedLagSolid::setTraction\n"
            "(\n"
...
void vertexCentredNonLinGeomUpdatedLagSolid::updateTotalFields()
{
    // Density
    //pointRho_ = pointRho_.oldTime()/pointRelJ_;
    Warning
        << "pointRho not updated! We need to calculate pointRelJ" << endl;

    // Update the dual mesh
    solidModel::clearDualMesh();
    dualMesh();

    // Update the pointVols
    // Set the pointVol and pointGlobalVol fields
    // Map dualMesh cell volumes to the primary mesh points
    scalarField& pointVolI = pointVol_;
    scalarField& pointGlobalVolI = pointGlobalVol_;
    ...
    // Note: D() is not used in the moveMesh function when "false" is provided
    // as the 4th argument (interpolateDDtoPointD)
    const vectorField oldPoints = mesh().points();
    solidModel::moveMesh(oldPoints, D(), pointDD(), false);

    solidModel::updateTotalFields();
}


void vertexCentredNonLinGeomUpdatedLagSolid::writeFields(const Time& runTime)
{
