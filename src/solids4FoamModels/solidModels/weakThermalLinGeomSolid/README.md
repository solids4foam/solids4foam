...
```warning
`weakThermalLinearGeometry` is **withdrawn**: it is not compiled on any fork,
so selecting it fails to construct the solid model. No tutorial selected it, so
nothing exercised it, and it was never ported to the mechanicalConstitutiveLaw
framework. The sources are kept and the entry is commented out in both build
lists. Reintroducing it needs a tutorial; see issue #459.
