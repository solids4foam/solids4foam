...
# Create container
CONTAINER="autobuild-solids4foam-openfoam-v9"
solids4foam::createContainer "openfoam-9" \
    "philippic/openfoam9-graphical-apps.gfortran:latest" \
    "${CONTAINER}" \
    "/home/openfoam"

# Build
if solids4foam::build "${CONTAINER}" "openfoam-9" \
...
# Test
if solids4foam::test "${CONTAINER}" "openfoam-9" \
       "/opt/openfoam9/etc/bashrc" "Alltest.openfoam"
then
    TEST_STATUS=true
else
    TEST_STATUS=false
