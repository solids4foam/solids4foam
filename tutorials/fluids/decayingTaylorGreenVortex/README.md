...
Prepared by Philip Cardiff and Ali Bozorgzadeh
...
- $$\mathbf{A} = (A_x, A_y, A_z)$$: magnitude/scale vector
    ...
    As can be seen from the plots, the value of the function $$f$$ increases
    as we go toward the middle of the domain in the $$x-y$$ plane. This trend
    is also
...
- $$\text{rand}_x, \text{rand}_y, \text{rand}_z$$: random numbers generated in
  $$x$$, $$y$$, and $$z$$ direction
...
The base case contains multiple `blockMeshDict` files of increasing mesh
 ...
 contains $$10 \times 10$$ cells. The `*.graded.*` versions of the
 `blockMeshDict`
...
According to the analytical solutions given above, the magnitude of the velocity
 ...
 of the velocity magnitude from $$t = 0$$ s to $$t = 0.4$$ s is shown in
 Video 1.
...
Re-running the case for different mesh densities allows the order of accuracy of
 ...
 where a spacing of $$0.05$$ m corresponds to the $$20 \times 20$$ grid
 described above ($$1$$ m divided into $$20$$ gives $$0.05$$ m).
