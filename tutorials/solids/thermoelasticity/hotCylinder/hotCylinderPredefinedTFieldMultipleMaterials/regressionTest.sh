...
check_arm() {
    ...
    if [[ "${expect_framework}" == yes ]]; then
        ...
        # last time too, not merely once at the start. OpenFOAM.com quotes
        # the path it logs and the other forks do not, so the quote is
        # optional; the end of the line is not, or time 4 would match 40
